...
public:
    ...
    CapPoint(int id,double x, double y,double theta,int dir,int collision,int dmcode) : id(id),x(x), y(y),theta(theta) ,dir(dir),collision(collision),dm_recgnize_enable(dmcode){};
    CapPoint(int id,double x, double y,double theta,int dir,int collision,int dmcode,int compens) : id(id),x(x), y(y),theta(theta) ,dir(dir),collision(collision),dm_recgnize_enable(dmcode),turn_compensation(compens){};
CapPoint(int id,double x, double y,double theta,int dir,int collision,int dmcode,int compens,int speed_s) : id(id),x(x), y(y),theta(theta) ,dir(dir),collision(collision),dm_recgnize_enable(dmcode),turn_compensation(compens),speed(speed_s){};
CapPoint(int id,double x, double y,double theta,int dir,int collision,int dmcode,int compens,int speed_s,int act,double rot_ang) : id(id),x(x), y(y),theta(theta) ,dir(dir),collision(collision),dm_recgnize_enable(dmcode),turn_compensation(compens),speed(speed_s),action(act),rotate_ang(rot_ang){};
	...
	int dir;
	int collision;
	int dm_recgnize_enable;
	int turn_compensation;////顶升旋转逆向补偿
	int speed;////该点的运行速度
	int action;/////该点的动作
	int rotate_ang;
	int id;
