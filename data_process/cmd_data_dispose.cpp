...
NavModule::NavModule(NavModuleOption &opt): option(opt) {
	...
	get_global_agv_instance()->exec_route|=(1<<16);
	
	printf("###NavModule::NavModule###\n");
...
void NavModule::cap_path_planning_new_if(CapRoute& cr){
	...
	//get_global_agv_instance()->lift_up_turn_switch=false;
        caps.push_back(Position(target.id,pose.x,pose.y,pose.theta));
        speed.push_back(0.5);
        dir.push_back(0);
        occl.push_back(1);
        inv_compensa.push_back(0);
        do_acts.push_back(0);
        fix_angs.push_back(0);
		int total_cnt = cr.cap_points.size();
		//printf("00000cap size=%d\n",total_cnt);
		char end_act = 0;
	get_global_agv_instance()->locate_modual->Set_dm_code_correct(false);

	float anti_speed = get_configs()->get_float("antiepidemic", "anti_speed", nullptr);
	for(itc=cr.cap_points.begin();itc!=cr.cap_points.end();itc++){
		...
		if((fabs(target.y - start.y)<0.1)&&(fabs(target.x - start.x)<0.1))
		target.theta = itc->theta;
		//if(target.collision==1)//开避障
		//get_global_agv_instance()->collision_switch=true;

		if(target.dm_recgnize_enable==1)//开二维码
		get_global_agv_instance()->locate_modual->Set_dm_code_correct(true);
		//if(target.turn_compensation==1)//开顶升旋转补偿
		//get_global_agv_instance()->lift_up_turn_switch=true;

        //printf("cur_id=%d/n",target.id);
        //caps.push_back(Position(target.id,itc->x,itc->y,target.theta));
        if(itc->action!=LIFT_NONE_CMD && (total_cnt>0)){
			//caps.push_back(Position(target.id,itc->x,itc->y,target.theta));
        caps.push_back(Position(target.id,itc->x,itc->y,itc->theta));
		//printf("9999999cur_id=%d\n",target.id);
		}
        else{
		end_act = (char)itc->action;
        caps.push_back(Position(target.id,itc->x,itc->y,itc->theta));
		//printf("77777777cur_id=%d\n",target.id);
		}
        //speed.push_back(anti_speed);
        if(itc->speed<10)
        speed.push_back(0.5f);
        else
        speed.push_back(itc->speed*0.001);
        dir.push_back((char)itc->dir);
        occl.push_back((char)itc->collision);
        inv_compensa.push_back((char)itc->turn_compensation);
        do_acts.push_back((char)itc->action);
        fix_angs.push_back(itc->rotate_ang*1.0);
        start=target;
	}
	char arg=0;
	switch(cr.op_id){
...
//(0-no action,1-up,2-down)
	if(get_global_agv_instance()->start_path(caps,speed,dir,end_act,occl,inv_compensa,do_acts,fix_angs)<0)
