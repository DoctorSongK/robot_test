...
//如果想要用备份带回卷的日志文件记录方式，就把这里的注释去掉
	log4cpp::RollingFileAppender* rollingfileAppender = new log4cpp::RollingFileAppender(
			"rollingfileAppender","/home/log/ale_task.log",20*1024,1);
...
{
	rootCategory.error(catMsg(msg));
}
void myLog::warn(const char* msg)
{
	rootCategory.warn(catMsg(msg));
}
void myLog::info(const char* msg)
{
	rootCategory.info(catMsg(msg));
}
void myLog::debug(const char* msg)
{
	rootCategory.debug(catMsg(msg));
}
void myLog::fatal(const char* msg)
{
	rootCategory.fatal(catMsg(msg));
}
void myLog::task_msg(const char* msg)
{
    rootCategory.info(taskMsg(msg));
}
...
myLog *loginstance = nullptr;

void init_log_instance()
{
    if (loginstance == nullptr)
    {
        loginstance = myLog::getInstance();
    }
}

myLog *get_log_instance()
{
    return loginstance;
}