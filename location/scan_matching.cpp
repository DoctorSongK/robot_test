...
public:
    OccupiedSpaceCostFunction2D(double scaling_factor, std::vector<Eigen::Vector2d> point_cloud, GlobalCoordinateMap * grid)
    ...
    bool operator()(const T* const pose, T* residual) const {
        ...
        for (int i = 0; i < size; i++) {
            ...
            interpolator.Evaluate(
                (world[0] - info.origen_x) / info.resolution,
                (world[1] - info.origen_y) / info.resolution,
                &residual[i]);
...
class TranslationDeltaCostFunctor2D {
    ...
    TranslationDeltaCostFunctor2D(
        double scaling_factor, double x, double y)
...
Position scan_matching(Position & init, GlobalCoordinateMap *map, PointCloudData &range_data, ScanMatchingOptions &option) {
    ...
    ceres::CostFunction* occupied_function = new ceres::AutoDiffCostFunction<OccupiedSpaceCostFunction2D,
        ceres::DYNAMIC /* residuals */,
        3 /* pose variables */>(
                new OccupiedSpaceCostFunction2D(option.occupied_space_cost_factor, range_data.points, map),
                range_data.points.size());
    problem.AddResidualBlock(occupied_function, nullptr /* loss function */, opt_buf);

    ceres::CostFunction* trans_delta_function = new ceres::AutoDiffCostFunction<TranslationDeltaCostFunctor2D,
        2 /* residuals */,
        3 /* pose variables */>(
                new TranslationDeltaCostFunctor2D(option.translation_delta_cost_factor, init.x, init.y));
    problem.AddResidualBlock(trans_delta_function, nullptr /* loss function */, opt_buf);

    ceres::CostFunction* rotation_delta_function = new ceres::AutoDiffCostFunction<RotationDeltaCostFunctor2D,
        1 /* residuals */, 
        3 /* pose variables */>(
                new RotationDeltaCostFunctor2D(option.rotation_delta_cost_factor, init.theta));
    problem.AddResidualBlock(rotation_delta_function, nullptr /* loss function */, opt_buf);
    ...
    return Position(init.timestamp, opt_buf[0], opt_buf[1], opt_buf[2]);
}
