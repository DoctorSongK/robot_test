...
double
angle_diff(double ang_a, double ang_b)
{
...
/*
void Amcl_gl::begin_high_resolution_map_create()
{
...
void Amcl_gl::end_high_resolution_map_create()
{
    begin_mapping = false;
	usleep(1000000);
    fclose(map_raw_data_file);
...
{   
    ...
    pthread_mutex_lock(&listen_pose_mutex);
	 ...
	 {//得到scanmatch
		...
		//radar_pos = expolate_current_position_raw();
...
bool Amcl_gl::para_initial()
{
	...
	laser_max_range = get_configs()->get_float("scan_matching", "max_distance", nullptr);
	if(laser_max_range<0) laser_max_range = 19.0;
	...
	laser_likelihood_max_dist = 0.3; //2.0地图上做障碍物膨胀的最大距离，用作likelihood_field模型
	...
	double offset_theta = get_configs()->get_float("radar", "radar_position_theta", nullptr);
	...
	ini_ang = from_index;
	printf("amcl radar_position_x=%f,radar_position_y=%f\n",offset_x,offset_y);
	end_ang = to_index;
	ang_step = 0.5;
	...
	para_LASER_2_BASE_A = offset_theta;
...
*/
	...
	{
//////////////////////////////////////////////////////////////机器人初始定位超时判断
	if(laser_recv_cnt<=5) {usleep(200000);continue;}
...
//// then start another locate thread
	{
	printf("stop the locating thread.11111111111\n");
	first_located_invoked = false;usleep(1000000);
	}
