...
enum {
    LIFT_NONE_CMD=0, LIFT_UP_CMD, LIFT_DOWN_CMD,LIFT_TURN_HOME_CMD,LIFT_ALL_HOME_CMD,LIFT_FIX_ANGLE_CMD,
} lift_cmd;/*顶升指令 */
#define LIFT_2D_UP_CMD     10
...
typedef struct 
{
    ...
    char is_occlli;////是否开启避障1-开避障，0-关避障
    char is_inverse_compensation;////是否开启逆向补偿1-开，0-关
    char do_action;////该点的动作0-无动作  1-顶升上升  2-顶升下降  3-顶升旋转回零  4-全部回零 5-转转固定角度
    float fix_ang;////旋转的角度
}T_TRAJ_;///////轨迹结构体
typedef struct 
{
...
class curve_run: public MsgQueueListener {
    ...
    void send_turn_msg(int dir, float angle,float ang_sp);
    char move_flag = 1;//////测试顶升用 0-无动作，1-上升，2-下降
public:
    pthread_t manu_run_thread;
    pthread_t curve_run_thread;
    pthread_t lift_up_thread;
    Position target_pos;
    bool busying = false;////机器人正在执行任务
    T_REALTIMEDATA	running_data;/////轨迹运行的实时数据
    int i_step;
    T_TRAJ_ CURBEZIER;
    vector <T_TRAJ_> myspace_traj;////从下发指令推算出的轨迹
    int set_path(vector <Position> pathpoints,vector <float> vel_points,vector <char> inverse_s,char do_act,vector <char> is_occ,vector <char> inverse_compensation,vector <char> point_act,vector <float> fix_ang);
    int initial_path(vector <Position> pathpoints,vector <float> vel_points,vector <char> inverse_t,char do_act,vector <char> is_occ,vector <char> inverse_compensation,vector <char> point_act,vector <float> fix_ang);
    Position cal_CZ(Position pos0,Position pos1,Position pos2,int &dir);////计算垂足点
    Position curp2curve(Position pcur,T_CURVE_P &curtraj,int curvetype,int &ret_flag);
    ////计算由当前点到曲线上距离最近的点
    Position find_initial_point( Position pcur,int &init_i, T_CURVE_P &curtraj,int curvetype,int &ret_flag);/////求解bezier曲线上的起始点坐标
    ...
    bool dis_among2(Position pos1,Position pos2,double dis);
    ...
    T_TRAJ_ ptp2curvetraj(Position ps,Position pe,float speed,char inverse_s,int traj_typ,char occ,char inverse_compensation,char point_act,float fix_ang);/////点到点组合成轨迹参数
    T_TRAJ_ ptp2traj(Position ps,Position pe,float speed,char inverse_t,int traj_typ,char occ,char inverse_compensation,char point_act,float fix_ang);/////点到点组合成轨迹参数
    ...
    bool task_compete;/////任务完成标识
    bool task_complete_send();/////任务完成后发送命令
    bool lift_task_run();/////仅仅启动顶升任务
    static void * _lift_up_thread_(void * param);
    Speed_data steer_speed;
    st_device_sts agv_sts;
    void set_manual_ratio(int speed_ratio);
    int get_manual_ratio();
private:
    ...
    bool zero_tuning = false;
    bool zero_turn = false;
    bool line_tuning = false;
    bool line_turn = false;
    float zero_turn_angle;
    char last_colli_flag;
    T_TRAJ_PARA Traj;////////////轨迹规划参数
    bool lift_turn_run_flag = false;//////顶升旋转启动标识
};
