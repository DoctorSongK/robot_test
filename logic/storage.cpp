	...
	if (!reader.parse(ifs2, route_json)){	//如果没有配置表文件,则建立
		...
		for(int i=0;i<route_json.size();i++){
			...
			for(int j=0;j<route_json[i]["caps"].size();j++){
					...
					route_json[i]["caps"][j]["dmcode"].asInt(),
					route_json[i]["caps"][j]["compensation"].asInt(),
					route_json[i]["caps"][j]["speed"].asInt(),
					route_json[i]["caps"][j]["action"].asInt(),
					route_json[i]["caps"][j]["rotateAng"].asInt());
...
void storage::insert_cap(CapPoint cap){
	...
	if(exist){
		...
		cap_json[i]["dmcode"]=cap.dm_recgnize_enable;
		cap_json[i]["compensation"]=cap.turn_compensation;
		cap_json[i]["speed"]=cap.speed;
		cap_json[i]["action"]=cap.action;
		cap_json[i]["rotateAng"]=cap.rotate_ang;
        ...
        capj["dmcode"]=cap.dm_recgnize_enable;
        capj["compensation"]=cap.turn_compensation;
		capj["speed"]=cap.speed;
		capj["action"]=cap.action;
		capj["rotateAng"]=cap.rotate_ang;
